...
class TabSeparatedParser : public QObject, ParserInterface
{
    Q_OBJECT
    Q_PLUGIN_METADATA(IID ParserInterface_iid FILE "TabSeparatedPlugin.json")
...
private:
    const std::regex peaksRegex = std::regex("([\\d\\.\\,]+)\t([\\d\\.\\,]+)", std::regex::icase);

    std::string customLocale = "C";
};

#endif // TABSEPARATEDPARSERINTERFACE_H
